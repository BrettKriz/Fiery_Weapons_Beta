...
function ENT:GiveDamage(target, bullet_ent)
	
			local pos = self.Entity:GetPos()
			local vn = self.Entity:GetVelocity():GetNormal()
			local bullet = bullet_ent -- better not be years
			local fx = EffectData()
				fx:SetOrigin(pos)
		if target:IsWorld() or IsEntity(target) or target:IsPlayer() or target:IsNPC() then
		
			-- stock_damage
			if GetConVar("swep_stock_damage"):GetBool() then -- Shoot a BULLET shot point blank
				local damage = self:GetAttackDamage()
				
				if bullet == nil then -- This is most likely where odd stuffs happening
					bullet = {}
					bullet.Num 		= num_bullets						-- Number
					bullet.Src 		= self.Entity:GetPos()				-- Source
					bullet.Dir 		= Angle(self.OrgAng) --* Angle( 0, 90, 0 )			-- Dir of bullet
					bullet.Spread 	= Vector( 0, 0, 0 )			-- Aim Cone
					bullet.Tracer	= 1									-- Show a tracer on every x bullets 
					bullet.Force	= damage/2							-- Amount of force to give to phys objects
					bullet.Damage	= damage							-- Damage
					bullet.AmmoType = self.AmmoType						-- Ammo Type
				end

				self:FireBullets( bullet ) -- END
				
				--{self.Owner, function(), self.Damage, self.Damage/2, 500, 1, 1, 1, "pistol", self.Entity:GetAngles(), Vector(0.01, 0.01, 0), self.Entity:GetPos()} -- END
			else
				target:TakeDamage(self:GetAttackDamage(), self.Owner, self.Owner:GetActiveWeapon()) 
				-- @@@EDIT THIS LATER!
